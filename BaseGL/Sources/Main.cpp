...
static const string SHADER_PATH ("Resources/Shaders/");
static const string DEFAULT_MESH_FILENAME ("Resources/Models/rhino2.off");
...
static shared_ptr<Camera> cameraPtr;
...
static vec3 baseTrans (0.0);
static vec3 baseRot (0.0);
...
// Lights:
unsigned int number_of_lights = 1;


namespace Texture {
static
GLuint loadTextureFromFileToGPU (const string & filename) {
...
struct FBO_ShadowMap {
    ...
    bool allocate( unsigned int width = 1024 , unsigned int height = 1024 ) {
        ...
        // glReadBuffer(GL_NONE); // Peut-être utile
        ...
        if(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
            ...
            cout << "PROBLEM IN FBO FBO_ShadowMap::allocate() : FBO NOT successfully created" << endl;
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
            return false;
        }
    }
...
public:
    ...
    void save_PPM_file(string const & filename ) {
        ofstream output_image(filename.c_str());
        ...
        output_image << "P3" << endl;
        output_image << depthMapTextureWidth << " " << depthMapTextureHeight << endl;
        output_image << "255" << endl;
...
struct Light {
    ...
    void setupCameraForShadowMapping(vec3 scene_center , float scene_radius) {
        // Compute depthMVP : the MVP matrix from the light's point of view
        float near_plane = 1.0f, far_plane = 7.5f;
        scene_radius /= 2.f;
        mat4 lightProjection = ortho(-scene_radius, scene_radius, -scene_radius, scene_radius, near_plane, far_plane);
...
struct Scene{
    ...
    shared_ptr<ShaderProgram> shaderProgramPtr;

    // a shader for the rendering of the shadow maps :
    shared_ptr<ShaderProgram> shadomMapShaderProgramPtr;
    ...
    void render() {
        ...
        for( int i = 0 ; i < scene_lights.size() ; ++i ) {
            ...
            if(save_shadow_maps_in_ppm) {
                light.shadowMap->save_PPM_file(string("shadom_map_")+to_string(i)+string(".ppm"));
            }
        }
        ...
        mat4 projectionMatrix = cameraPtr->computeProjectionMatrix ();
        ...
        mat4 modelMatrix = meshPtr->computeTransformMatrix ();
        mat4 viewMatrix = cameraPtr->computeViewMatrix ();
        ...
        {
            shaderProgramPtr->set (string("number_of_lights"), (int) scene_lights.size());

            for (int index_light = 0; index_light < scene_lights.size(); index_light++){
                Light & light = scene_lights[index_light];
                shaderProgramPtr->set (string("lightSource[")+to_string(index_light)+string("].position"), light.m_position);
                shaderProgramPtr->set (string("lightSource[")+to_string(index_light)+string("].color"), light.m_color);
                shaderProgramPtr->set (string("lightSource[")+to_string(index_light)+string("].intensity"), light.m_intensity);
                shaderProgramPtr->set (string("lightSource[")+to_string(index_light)+string("].depthMVP"), light.depthMVP);
                shaderProgramPtr->set (string("lightSource[")+to_string(index_light)+string("].isActive"), 1);
                glActiveTexture(GL_TEXTURE1);
                glBindTexture(GL_TEXTURE_2D, light.shadowMap->depthMapTexture);
                shaderProgramPtr->set (string("shadowMap[")+to_string(index_light)+string("].depthMap"), 1);
                shaderProgramPtr->set (string("shadowMap[")+to_string(index_light)+string("].lightIndex"), index_light);
            }
        }

        // Set the material for the plane mesh :
        shaderProgramPtr->set ("material.albedo", vec3( 0.5 , 0.5 , 0.5 ) );
        ...
        shaderProgramPtr->set ("material.albedo", vec3( 0.4 , 0.4 , 0.8 ) );
...
void printHelp () {
    cout << "> Help:" << endl
              << "    Mouse commands:" << endl
              << "    * Left button: rotate camera" << endl
              << "    * Middle button: zoom" << endl
              << "    * Right button: pan camera" << endl
...
void cursorPosCallback(GLFWwindow* window, double xpos, double ypos) {
    ...
    if (isRotating) {
        vec3 dRot (-dy * M_PI, dx * M_PI, 0.0);
        cameraPtr->setRotation (baseRot + dRot);
    }
    else if (isPanning) {
        cameraPtr->setTranslation (baseTrans + meshScale * vec3 (dx, dy, 0.0));
    } else if (isZooming) {
        cameraPtr->setTranslation (baseTrans + meshScale * vec3 (0.0, 0.0, dy));
    }
}
...
void initGLFW () {
    ...
    if (!windowPtr) {
        cerr << "ERROR: Failed to open window" << endl;
        glfwTerminate ();
        exit (EXIT_FAILURE);
    }
...
void exitOnCriticalError (const string & message) {
    cerr << "> [Critical error]" << message << endl;
    cerr << "> [Clearing resources]" << endl;
    clear ();
    cerr << "> [Exit]" << endl;
    exit (EXIT_FAILURE);
}
...
void initOpenGL () {
    ...
    try {
        ...
        exitOnCriticalError (string ("[Error loading shader program]") + e.what ());
    }
    try {
...
void initScene (const string & meshFilename) {
    // Init camera
    int width, height;
    glfwGetWindowSize (windowPtr, &width, &height);
    ...
    {
        scene.meshPtr = make_shared<Mesh> ();
        try {
            MeshLoader::loadOFF (meshFilename, scene.meshPtr);
        } catch (exception & e) {
            exitOnCriticalError (string ("[Error loading mesh]") + e.what ());
        }
        scene.meshPtr->init ();

        scene.planeMeshPtr = make_shared<Mesh> ();
    ...
    // Setup lights :
    {
        unsigned int shadow_map_width = 2000 , shadow_map_height = 2000; // play with these parameters
        {
            for (int index_light = 0; index_light < number_of_lights; index_light++){
                ...
                newLight.m_position = vec3 (-2*cos(index_light), 2*sin(index_light), 3.5);
                newLight.m_color = vec3(1.0, 1.0, 1.0);
                newLight.m_intensity = 0.5f /* * (1.f / (float) number_of_lights) */;
                cout<<"("<<newLight.m_position[0]<<", "<<newLight.m_position[1]<<", "<<newLight.m_position[2]<<")"<<endl;
    ...
    cameraPtr->setTranslation (scene.scene_center + vec3 (0.0, 0.0, 3.0 * meshScale));
    cameraPtr->setNear (meshScale / 100.f);
    cameraPtr->setFar (6.f * meshScale);
}

void init (const string & meshFilename) {
...
void update (float currentTime) {
    if( ! app_timer_is_stopped ) {
        ...
        light0.m_position = vec3 (-2*cos(app_timer), 2*sin(app_timer), 3.5);
    }
}

void usage (const char * command) {
    cerr << "Usage : " << command << " [<file.off>]" << endl;
    exit (EXIT_FAILURE);
}
